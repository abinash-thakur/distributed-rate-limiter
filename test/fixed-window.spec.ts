...
describe('Fixed window algorithm', () => {
    ...
    it('allows requests under the limit', async () => {
        const result = await service.check(RateLimitAlgorithmEnum.FIXED_WINDOW, 'test:fw:1', 5, 60);
    ...
    it('denies request exactly at the limit', async () => {
        const key = 'test:fw:2';
        for (let i = 0; i < 5; i++) {
            await service.check(RateLimitAlgorithmEnum.FIXED_WINDOW, key, 5, 60);
        }
        const result = await service.check(RateLimitAlgorithmEnum.FIXED_WINDOW, key, 5, 60);
